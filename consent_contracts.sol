 ...
 *   - GDPR Article 7 compliant withdrawal mechanism
 ...
 *   - GDPR Article 17 - Right to erasure (cryptographic deletion)
...
contract MedIntelOSConsentManager {
    ...
    struct ConsentRecord {
        ...
        string fhirPatientId;      // Off-chain FHIR Patient resource ID
    ...
    mapping(address => bytes32[]) public patientConsents;        // patient → consentIds
    mapping(address => bytes32[]) public requesterConsents;      // requester → consentIds
    mapping(address => InstitutionProfile) public institutions;
    mapping(bytes32 => EmergencyOverride) public emergencyOverrides;
    mapping(address => bool) public authorizedOracles;           // FHIR bridge oracles
     ...
     * @param fhirPatientId Off-chain FHIR Patient resource ID
    ...
    function grantConsent(
        ...
        string calldata fhirPatientId,
        bytes32 irbApprovalHash
    ) external onlyVerifiedInstitution returns (bytes32 consentId) {
        require(
        ...
        institutions[requester].consentCount++;
        ...
        if (auditLedgerAddress != address(0)) {
            IAuditLedger(auditLedgerAddress).logConsentEvent(
                consentId, msg.sender, requester, "GRANTED",
                string(abi.encodePacked(dataScope[0]))
            );
        }

        emit ConsentGranted(consentId, msg.sender, requester, purpose, expiresAt);
        return consentId;
    }
    ...
    function grantProxyConsent(
        ...
        // In production: verify legal proxy status via oracle or off-chain document hash
        uint256 expiresAt = durationDays > 0
            ? block.timestamp + (durationDays * 1 days)
            : 0;
        ...
        emit ConsentGranted(consentId, patient, requester, purpose, expiresAt);
        return consentId;
    }
