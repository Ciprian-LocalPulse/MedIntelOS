...
Features:
    ...
    - Rule contribution details suitable for human review
...
class ClinicalAlert:
    ...
    explanation: Optional[Dict[str, Any]] = None  # Rule contribution details
...
class RiskScorer:
    """
    Educational implementations of published clinical scoring rules.

    These implementations have not been clinically validated as a software
    system and must not be used for diagnosis or treatment decisions.
    """
    ...
    def cha2ds2_vasc(
        ...
        if has_chf:
            score += 1
            explanation["congestive_heart_failure"] = 1.0
        if has_hypertension:
            score += 1
            explanation["hypertension"] = 1.0
        if age >= 75:
            score += 2
            explanation["age_ge_75"] = 2.0
        elif age >= 65:
            score += 1
            explanation["age_65_74"] = 1.0
        if has_diabetes:
            score += 1
            explanation["diabetes"] = 1.0
        if has_stroke_tia:
            score += 2
            explanation["prior_stroke_tia"] = 2.0
        if has_vascular_disease:
            score += 1
            explanation["vascular_disease"] = 1.0
        if sex.lower() == "f":
            score += 1
            explanation["female_sex"] = 1.0
...
class CDSSEngine:
    ...
    def evaluate(
        ...
        if context.vitals:
            ...
            if qsofa.risk_level in (RiskLevel.HIGH, RiskLevel.CRITICAL):
                all_alerts.append(ClinicalAlert(
                    priority=AlertPriority.CRITICAL,
                    title=f"Sepsis screening alert: qSOFA score {int(qsofa.score_value)}/3",
                    detail=(
            ...
            if news2.score_value >= 7:
                all_alerts.append(ClinicalAlert(
                    priority=AlertPriority.CRITICAL,
                    title=f"NEWS2 score {int(news2.score_value)} - urgent review",
                    detail=(
        ...
        # --- AKI Detection ---
        creatinine_results = [
            lab for lab in context.labs if lab.loinc_code in ("2160-0", "38483-4")
        ]
        if creatinine_results:
            ...
            if aki.risk_level in (RiskLevel.HIGH, RiskLevel.CRITICAL):
                all_alerts.append(ClinicalAlert(
                    priority=AlertPriority.WARNING,
                    title=f"Possible AKI stage {int(aki.score_value)} (KDIGO rule)",
                    detail=(
        ...
        # --- Critical Lab Values ---
        critical_labs = [lab for lab in context.labs if lab.is_critical]
        for lab in critical_labs:
            all_alerts.append(ClinicalAlert(
                priority=AlertPriority.CRITICAL,
                title=f"Critical lab threshold: {lab.display_name} = {lab.value} {lab.unit}",
                detail=(
